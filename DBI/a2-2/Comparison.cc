...
OrderMaker :: OrderMaker(ifstream &fin)
{
    int type;
  	fin >> numAtts;
...
void OrderMaker :: WriteToFile(ofstream &fout) {
	fout << numAtts << "\n";
	for (int i = 0; i < numAtts; i++) {
		fout << whichAtts[i] << " " << (int)whichTypes[i] << "\n";
	}
}

int OrderMaker::GetAttribute(int position) {
	return whichAtts[position];
}

Type OrderMaker::GetType(int position) {
	return whichTypes[position];
}

int OrderMaker::GetNumOfAtts() {
	return numAtts;
}

void OrderMaker::AddAttribute(int att, Type type) {
	whichAtts[numAtts] = att;
