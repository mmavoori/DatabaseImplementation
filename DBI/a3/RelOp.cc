...
//Read the input form the input DBFile and apply CNF on every tuple, 
//whichever record accepts put it into out pipe
...
//Read the input form the input pipe and apply CNF on every tuple, 
//whichever record accepts put it into out pipe
...
//Select only the keepMe attributes from the inpipe and place them onto outpipe.
...
void* ProjectWorker(void* iProjectObj){
	...
	Record lTempRecord;
    while(lConversionObj->mInPipe->Remove(&lTempRecord)) {
      //Using the Project function of Record Class
...
//Calculate the Sum aggregate function depending on the type of the input attribute
//of the record from the inpipe and place the Sum onto the outpipe
...
//The inPipe records are ordered using the BigQ. After sorting, two consecutive records are
//compared. If equal Sum up the value. If not equal place one group onto outpipe and increase 
//group number to work with remaining records.
...
//Records from inPipe are sorted using BigQ. The previous and current records 
//are compared with each other. If equal eliminate. If not equal write to outpipe. 
